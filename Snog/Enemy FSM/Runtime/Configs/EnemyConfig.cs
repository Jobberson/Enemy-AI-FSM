...
{
    /// <summary>
    /// Global configuration for all enemy behaviors:
    ...
    public class EnemyConfig : ScriptableObject
    {
        ...
        [Tooltip("Speed when investigating noise.")]
        public float investigateSpeed = 8f;

        [Tooltip("Base speed when chasing.")]
        public float chaseSpeed = 10f;

        [Tooltip("Speed when recovering.")]
        public float recoverSpeed = 4f;

        [Header("Vision & Hearing")]
        [Tooltip("Max distance eyes can see.")]
        public float viewDistance = 10f;

        [Tooltip("Field of view angle (in degrees).")]
        public float viewAngle = 60f;

        [Tooltip("Seconds required to confirm sighting.")]
        public float timeToSpotPlayer = 1f;

        [Tooltip("Radius within which noises are detected.")]
        public float noiseDetectionRadius = 8f;

        [Header("Chase Lose Settings")]
        [Tooltip("Time (sec) to give up chase after losing sight.")]
        public float chaseLoseTime = 3f;

        [Header("Intensity Increase Limits")]
        [Tooltip("Min recover duration after intensity increase.")]
        ...
        [Tooltip("Min time-to-spot-player allowed after intensity increase.")]
        public float minTimeToSpotPlayer = 0.5f;

        [Tooltip("Max wander speed allowed after intensity increase.")]
        public float maxWanderSpeed = 8f;

        [Tooltip("Max chase speed allowed after intensity increase.")]
        public float maxChaseSpeed = 12f;

        [Tooltip("Max investigate speed allowed after intensity increase.")]
        public float maxInvestigateSpeed = 10f;

        [Tooltip("Max bias chance (%) allowed after intensity increase.")]
        public float maxBiasChancePercentage = 75f;
